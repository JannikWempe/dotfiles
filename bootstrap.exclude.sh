...
install_tools () {
	if [ $( echo "$OSTYPE" | grep 'darwin' ) ] ; then
		...
		if [ "$resp" = 'y' -o "$resp" = 'Y' ] ; then
			...
			sh brew.exclude.sh
			echo "$PROMPT Finished Homebrew installations"
			echo "$PROMPT If this is an initial macOS setup you might want to run ~/.macos"
		else
			echo "$PROMPT Homebrew installation cancelled by user"
		fi
	else
